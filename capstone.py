...
# # waste_types = ['vegetable_peels', 'fruit_pulp','Cassava_peels', 'grain', 'meat', 'dairy']

# # n = 1000  # number of waste listings

# # # Generate synthetic waste data
# # waste_data = pd.DataFrame({
# #     'waste_id': range(1, n+1),
# #     'waste_type': np.random.choice(waste_types, n),
# #     'quantity_kg': np.random.randint(50, 2000, n),
# #     'moisture_level': np.random.uniform(0.2, 0.9, n),
# #     'contamination': np.random.choice([0, 1], n, p=[0.8, 0.2]),
# #     'perishability_hours': np.random.randint(6, 72, n),
# #     'latitude': np.random.uniform(6.4, 6.7, n),
# #     'longitude': np.random.uniform(3.2, 3.5, n)
# # })
...
# buyer_types = ['Animal Feed', 'Compost', 'Bioenergy', 'Biofuel']

# m = 200

# # Subcategory acceptance mapping
# acceptance_map = {
#     "Animal Feed": ["Grain Husk", "Root/Tuber Peels", "Bakery Surplus", "Dairy Whey"],
...
#     accepted_subcategories = acceptance_map[buyer_type]

#     # Choose ONE buyer type (don't overwrite the list)

    
#     record = {
#         "buyer_id": i + 1,
#         "buyer_type": buyer_type,
#         "accepted_subcategories": ", ".join(accepted_subcategories),  # convert list to string
#         "max_capacity_kg": np.random.randint(500, 5000),
#         "max_distance_km": np.random.randint(5, 50),
#         "demand_level": round(np.random.uniform(0.3, 1.0), 2),
#         "latitude": np.random.uniform(6.4, 6.7),
#         "longitude": np.random.uniform(3.2, 3.5),
#         "current_available_capacity_kg": np.random.randint(0, 5000)
#     }
...
# # Save correctly
# df_buyers.to_csv("buyer_data.csv", index=False)
